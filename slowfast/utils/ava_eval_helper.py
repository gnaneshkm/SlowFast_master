...
def evaluate_ava_from_files(labelmap, groundtruth, detections, exclusions):
    ...
    run_evaluation(categories, groundtruth, detections)
...
def evaluate_ava_train(
    ...
    write_results(detections, "detections_train%s.csv" % name)
    write_results(groundtruth, "groundtruth_train%s.csv" % name)



def run_evaluation(
    categories, groundtruth, detections, verbose=True
):
...
def write_results(detections, filename):
    #print ("the file name is {} and {} are".format(filename ,detections))
    ...
    for key in boxes.keys():
        for box, label, score in zip(boxes[key], labels[key], scores[key]):
            print (f'the box is {box},and label for box is {label},and score is {score}')
